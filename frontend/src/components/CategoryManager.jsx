...
export default function CategoryManager() {
  ...
  async function handleSubmit(e) {
    ...
    let latest = categories;
    for (const n of names) {
      try {
        latest = await addCategory(n);
    ...
    <div className="max-w-lg space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-slate-700">All Categories</h3>
        ...
        {categories.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map((c) => (
              <span key={c} className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium ${categoryColor(c)}`}>
                {c}
              </span>
            ))}
          </div>
      ...
      {/* Add form */}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
        <h3 className="text-sm font-semibold text-slate-700 mb-3">Add Categories</h3>
        <p className="text-xs text-slate-400 mb-3">Separate multiple names with commas — e.g. <span className="text-slate-500 font-medium">Blinkit, Dog Food</span></p>
